export function NewVaccineForm () {
  ...
  const insertVaccine=(vaccine_details)=>{
    console.log(vaccine_details,JSON.stringify( vaccine_details ))
    var requestOptions = {
        method: 'POST',
        headers: myHeaders,
        mode: 'cors',
        redirect: 'follow',
        body: JSON.stringify( vaccine_details )  
      };

      fetch("http://localhost:3000/insertVaccine", requestOptions)
    ...
    const handleSubmit = (event) => {
        ...
        for (const value of data.entries()) {
            vaccine_details[value[0]]=value[1]

          console.log(value);
        }
        
        insertVaccine(vaccine_details)
